...
router.post("/inquiry", upload.single("file"), (req, res) => {
  ...
  // 업로드된 파일이 있다면 웹에서 접근 가능한 경로로 저장
  let filePath = req.file ? "/uploads/" + req.file.filename : null;

  // QUESTION_INFO 테이블 컬럼: QUES_IDX (AUTO_INCREMENT), QUES_TITLE, QUES_CONTENT, QUES_FILE, CREATED_AT, QUES_VIEWS, USER_ID
  const sql = `
    INSERT INTO QUESTION_INFO
      (QUES_TITLE, QUES_CONTENT, QUES_FILE, CREATED_AT, QUES_VIEWS, USER_ID)
    VALUES
      (?, ?, ?, NOW(), 0, ?)
  `;
  conn.query(sql, [title, content, filePath, userId], (err, result) => {
    if (err) {
